...
import { fetchLatestResearch, generateDeepSummary, generateWeeklyReport } from './services/geminiService';
...
const App: React.FC = () => {
  ...
  const loadWeeklyReport = async () => {
    ...
    try {
      ...
      const report = await generateWeeklyReport(data, range.start, range.end);
      setWeeklyReport(report);
    } catch (err: any) {
      console.error("Error generating weekly report:", err);
      setError(err.message || "리포트 생성 중 오류가 발생했습니다.");
      setWeeklyReport("리포트 생성 중 오류가 발생했습니다.");
    } finally {
      ...
      <div className="flex items-center justify-center mb-12">
        <div className="bg-slate-100 p-1.5 rounded-2xl flex items-center shadow-inner">
          ...
          <button 
            ...
            className={`px-8 py-3 rounded-xl text-sm font-black transition-all ${viewMode === 'weekly' ? 'bg-white text-blue-600 shadow-md' : 'text-slate-500 hover:text-slate-700'}`}
          >
            주간 브리핑
          </button>
        </div>
        ...
        <div className="max-w-4xl mx-auto space-y-12 animate-in fade-in slide-in-from-bottom-6 duration-1000">
          <div className="bg-white rounded-[3rem] border border-slate-200 shadow-xl overflow-hidden">
            <div className="bg-slate-900 px-8 py-12 md:px-16 text-white text-center">
              <h3 className="text-3xl font-black mb-4">주간 연구 브리핑</h3>
            ...
            <div className="p-8 md:p-16">
              ...
              {isWeeklyLoading ? (
                <div className="flex flex-col items-center justify-center py-20 space-y-8">
                  ...
                  <p className="text-slate-900 font-black text-xl">지난 한 주간의 데이터를 분석 중입니다...</p>
                </div>
              ) : (
                <div className="prose prose-slate max-w-none prose-headings:font-black prose-h3:text-blue-600 prose-p:text-slate-600 prose-p:leading-relaxed prose-strong:text-blue-600 prose-a:text-blue-600 hover:prose-a:text-blue-700 prose-a:no-underline hover:prose-a:underline transition-all">
